...
export const fetchQuestionAttemptResults = async (
    id: string,
    user_answer: string
): Promise<QuestionAttemptResultsAttributes> => {
    // server will decide the next question to fetch
    console.log("XXXXXX IN fetchQuestionAttemptResults _id", id, "user answer", user_answer)
    ...
    //console.log("fetchQuestionAttemptResults url", url)
