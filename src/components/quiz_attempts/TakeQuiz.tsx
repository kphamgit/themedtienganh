...
export default function TakeQuiz() {
   ...
   // console.log("user in TakeQuiz=", user)
    ...
    const { data: quizAttempt } = useQuizAttempt(params.quizId!, user?.id?.toString() ?? "", fetchQuizAttemptEnabled)
...
const get_next_question = async () => {
    ...
    const handleSegmentEnd = () => {
        console.log("Video Segment ended, do something here, e.g.,  show quiz questions");
   ...
   const handleSubmit: MouseEventHandler<HTMLButtonElement> = (event) => {
    ...
    return (
        ...
        <h2>{endOfQuiz}</h2>
            <div><YoutubeVideoPlayerNew video_url={quiz?.video_url || ""} video_segments={videoSegments.current}  parentCallback={handleSegmentEnd} /></div>
